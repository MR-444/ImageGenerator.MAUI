...
- **One configurable data root.** All app data now lives under a single root folder (default `Pictures\Emberforge`), laid out as:
  - `Pictures\Emberforge\pictures\` — generated images (now a subfolder; they used to sit loose in the root)
  - `Pictures\Emberforge\json-prompts\`
  - `Pictures\Emberforge\comfy-workflows\`
  - `Pictures\Emberforge\mutation-library\`
  - `Pictures\Emberforge\prompt-builder\`

  When you point the output folder elsewhere in **Settings**, the whole set moves with it. Previously `comfy-workflows\`, `mutation-library\`, and `prompt-builder\` stayed pinned to the default location, which stranded your ComfyUI workflows and made the **ComfyUI provider silently disappear** from the model picker when you used another drive. Fixed.
...
1. **Your data folder.** In v2.0.0 everything lived under `Pictures\ImageGenerator.MAUI\` — images loose in that folder, plus `json-prompts\` / `comfy-workflows\` / `mutation-library\` / `prompt-builder\` subfolders. To carry it over:
   - Move the loose **image files** from `Pictures\ImageGenerator.MAUI\` into the new `Pictures\Emberforge\pictures\` subfolder (note: images now live one level deeper than before).
   - Move the `json-prompts\`, `comfy-workflows\`, `mutation-library\`, and `prompt-builder\` subfolders into `Pictures\Emberforge\` (same names, one level up from the images).
   - Or, simplest: rename `Pictures\ImageGenerator.MAUI` to `Pictures\Emberforge`, then move just the loose image files down into its new `pictures\` subfolder.
